...
- LAN conversation只透過`lan_secure_demo` HTTPS開放。真實資料仍須通過guest／user owner scope、Cloud Processing Notice、private storage與secret hard block；不得宣稱能完整偵測PII。Fixture不發OpenAI，Live需顯式Cloud／Project Gate，client仍不得直報domain state。
...
- 一般PII疑慮採warning後可繼續：Server回`PII_WARNING_REQUIRED`，ack綁actor／case／revision／payload hash、10分鐘且單次；不得預勾／自動重送，也不得宣稱完整偵測。Password、OTP／reset code、API key、Authorization／session token、private key等auth secrets，以及完整金融帳號／QR／data URL仍`AUTH_SECRET_DETECTED` hard block，不可ack繞過、保存、log或送模型。
