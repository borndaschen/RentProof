...
- 狀態：Provider、模型路由與Secure LAN Live Gate已接入
...
- 目前case budget：16 provider attempts、concurrency 2、500K input tokens、50K output＋reasoning tokens、US$2 engineering alert
...
截至2026-09-02，OpenAI Docs列出的Terra標準價格為每1M tokens：input US$2、cached input US$0.20、output US$12。目前採寬鬆案件上限：最多16次provider attempts（包含SDK實際重試）、同時2個requests、累計500,000 input tokens與50,000 output＋reasoning tokens，應用層US$2／案件為工程警戒而非帳單保證。價格變更時更新估算，但不自動放寬token／attempt caps。
...
所有目前呼叫設定 `store: false`。這表示應用不依賴 Responses API 保存 response 供日後 retrieve，但**不等於 Zero Data Retention**，也不代表所有監控資料立即刪除。
...
Prompt caching 也可能在 GPU-local storage 保存加密 application state；background mode 會為 polling 暫存 response。目前不使用 background，且不對使用者承諾 OpenAI「完全不保存」。

因此：

- Golden／Fixture流程不呼叫OpenAI；`lan_secure_demo`只有在使用者確認Cloud Processing Notice後才可送出私有案件的最小必要內容。
- Secure LAN Live啟動需`RENTPROOF_LLM_MODE=live`、server-only `OPENAI_API_KEY`及`OPENAI_PROJECT_LIMITS_CONFIRMED=true`；缺少任一條件即fail closed。
- 介面已在建立私有案件前告知資料會送往OpenAI Cloud並取得明確確認；每次分析仍必須遵守資料最小化與owner scope。
...
Development Project月額US$100 Hard Spend Limit及US$50／US$80 alerts已由操作者確認設定。Runtime仍以`OPENAI_PROJECT_LIMITS_CONFIRMED=true`作每次Secure LAN Live啟動的顯式Gate；此值只代表操作者確認，不取代Dashboard帳單與速率限制。Admin API key不進RentProof runtime、env example、CI或server；App只使用該Project的scoped service key。
...
- `docs/SECURITY_PRIVACY.md` 的OpenAI與目前Security Gate全部通過。
