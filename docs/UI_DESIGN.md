...
- 首頁直接以對話依序引導案件名稱、租屋廣告、看屋照片／受限MP4與租約；登入／註冊是保存與查詢歷史的選用功能。
...
- Real Data首頁以有邊界的對話區塊承載流程，但不另顯示「對話整理／租屋資料對話」重複標題；第一個可見內容直接是RentProof回覆。短對話時composer貼齊區塊底部，長內容時sticky於viewport下方。Desktop整個composer接受檔案drop，Mobile以48px「＋」開啟附件；Server仍驗MIME、大小與目前stage，不信任drop或accept屬性。
- Real Data首頁不在訊息串插入大型租屋補貼卡片，補貼預檢維持header導覽入口；每一則Server／Agent訊息都顯示`RentProof` label、左側accent與淡色背景，訪客保存提醒與狀態訊息不得呈現成無歸屬的一般段落。
- 尚未建立案件時只顯示一個RentProof初始回覆框；開始方式與必要的訪客保存提醒合併在同一則，不把相同說明拆成連續三個回覆。案件建立後才依新事件追加狀態或下一步訊息。
- 租屋補助頁在使用者按下「查看預檢結果」且Server成功回傳前，不顯示「結果怎麼看」或三種結果狀態；結果與解釋只在完成一次預檢後一起出現。
- 登入後header固定顯示三個同尺寸、同外框控制：「租屋補助預檢」「我的案件」「登出」；不再顯示語意不明的「帳戶」。登出成功後清除頁面中的帳戶案件projection並建立新的訪客session，避免殘留已無權存取的畫面。
- Guest案件只有在原分頁偵測到同瀏覽器已登入帳戶後才顯示「保存」。登入使用新分頁，focus返回後刷新帳戶與CSRF狀態；保存失敗不得鎖住「刪除」，Server delete需同時安全解析account／guest actors並只刪除實際owner案件。
