...
| 輸入                   | 內容                                                                           | 安全限制                                                     |
| ---------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| `interaction_evidence` | 完全虛構的聊天文字／截圖，包含看屋與付款要求                                   | P0 synthetic only；遮蔽姓名、頭像、電話、帳號、QR code       |
...
P0 不上傳真實身分證、權狀或銀行資料。若需要確認出租權限，只保存「已確認／未確認／未知」與查證方式，不保存證件影像。
...
| ID        | Phase | 訊號                                                    | 最小觸發資料                                                  | 行動                    |
| --------- | ----- | ------------------------------------------------------- | ------------------------------------------------------------- | ----------------------- |
| `FRS-001` | P0    | 首次實地看屋前要求訂金、預約金、鑰匙押金或租金          | 可定位付款要求＋`paymentRequestedAt < firstInPersonViewingAt` | `stop_and_verify`       |
| `FRS-002` | P1    | 自稱人在國外／外地、拒絕當面帶看或只能寄送鑰匙          | 可定位對話文字                                                | `verify_before_payment` |
| `FRS-003` | P1    | 要求點陌生物流／超商／客服連結，或提供網銀、信用卡、OTP | 可定位連結／要求語意                                          | `stop_and_verify`       |
| `FRS-004` | P1    | 收款人與出租人、代理人或契約當事人關係不明／不一致      | 角色名稱或使用者手動確認；不需完整帳號                        | `stop_and_verify`       |
| `FRS-005` | P1    | 付款前仍未核對出租權限                                  | `authority_verified = false                                   | unknown`＋存在付款要求  | `verify_before_payment` |
| `FRS-006` | P1    | 「其他人正在搶租」「現在匯款才能保留」等高壓稀缺話術    | 可定位對話文字                                                | `verify_before_payment` |
| `FRS-007` | P1    | 要求跨境匯款、加密貨幣、禮物卡或其他難追回方式          | 可定位付款方式                                                | `stop_and_verify`       |
| `FRS-008` | P1    | 廣告、現場、契約、付款要求或當事人角色出現明確矛盾      | 已驗證 evidence refs                                          | `verify_before_payment` |
| `FRS-009` | P1    | 租金顯著低於官方租金脈絡，且同時存在其他付款／身分訊號  | 官方價格脈絡＋至少一項其他 detected signal                    | `verify_before_payment` |
| `FRS-010` | P1    | 被導向陌生客服／LINE，要求帳戶認證、身分資料或操作網銀  | 可定位對話／連結語意                                          | `stop_and_verify`       |

`FRS-009` 不得由低租金單獨觸發；價格脈絡不能證明詐騙，也不能輸出租金合理／不合理。

單人 P0 只實作 `FRS-001`。若付款要求或首次實地看屋時間任一未知，結果必須是 `insufficient_information`；不能用「尚未看到看屋證據」推論付款一定發生在看屋前。

## 6. LLM 與確定性規則分工

OpenAI `gpt-5.6-terra` 只負責：

- P0 從 synthetic 互動文字抽取付款要求與 locator；P1 才擴充付款方式、高壓話術、遠端房東、寄送鑰匙、陌生連結與角色名稱候選。
...
外部 `RentProof-Demo/interaction/` 新增一份 synthetic 對話：
...
Demo 不說「已確認詐騙」，也不自動撥號、報案、封鎖對方或通報帳戶。

## 10. 安全與驗收 Gate

- Synthetic-only；不含真實聊天、姓名、電話、帳號、OTP、QR code 或身分文件。
- Interaction upload 通過 magic bytes、realpath、EXIF、大小與 HTML escaping。
- 陌生 URL 只作不可點擊／安全顯示文字；P0 不 fetch、不開啟、不做網路查詢。
