...
模組、目錄、依賴方向、ports／adapters、stage DAG、runtime layout與部署邊界依[系統架構](SYSTEM_ARCHITECTURE.md)實作；本機HTTP與LAN HTTPS listener依[Server配置](SERVER_CONFIGURATION.md)。Self-hosted Auth／PostgreSQL以feature-gated adapters接入；正式服務的guest／user sessions、歷史與政策契約仍依[選用帳戶、登入與歷史租約架構](AUTH_AND_HISTORY.md)。
...
| C. 應用骨架       | 在目前Windows桌面電腦使用Node.js 24 LTS＋pnpm Scaffold Next.js 16 Active LTS／TypeScript 6.0／Vitest，配置ESLint Flat Config＋Prettier、server-only OpenAI config、Gateway interface、fixture mode、loopback HTTP、`lan_secure_demo`與conversation-first shell | `package.json.private=true`且專案採Apache-2.0 `LICENSE`／`NOTICE`；Node、Next、TypeScript與pnpm鎖版；`.env.local` ignored／ACL最小、secret scan通過；lint／format check／typecheck獨立執行；TLS、Windows path／NTFS／Firewall smoke通過；client bundle無key；LAN public／wildcard bind fail closed |
...
| RP-DEV-003 | P0  | 001             | 以原生Node.js 24 LTS＋pnpm Scaffold Next.js 16 Active LTS／TypeScript／Vitest，不引入Docker／WAMP proxy／Windows Service；鎖定工具版本，加入validated launcher、OpenAI server config、live／fixture、loopback與`lan_secure_demo` profiles；日常Dev Server、LAN展示Production Build，但`NODE_ENV`不得提升profile能力                                                                                                                 |
...
- 同一private LAN的Mobile／Desktop只透過`lan_secure_demo` HTTPS使用；舊HTTP LAN profile拒絕啟動。
...
- LAN展示以Production Build＋`lan_secure_demo`啟動，無HMR／source maps／詳細error overlay；Fixture／Live與Auth／storage能力由validated設定決定。
- 外部素材、環境設定與還原方式已有文件。
...
- 開發server的HTTP固定loopback；`lan_secure_demo`以HTTPS private-IP運行，TLS／Firewall／Host／Origin／Secure Cookie／owner與private storage checks完成。
